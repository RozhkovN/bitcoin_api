...
// startAfterTxid: если непуст — начинаем с TX старее этого txid (backward sync из кэша).
// При падении источника N-й tx → передаёт cursor следующему источнику.
func fetchBtcTxViewsRelay(ctx context.Context, addr string, want int, startAfterTxid string) ([]BtcTxView, error) {
	var all []BtcTxView
	cursor := startAfterTxid // txid курсора; пустой = с самого начала
