...
func analyzeBitcoinRich(ctx context.Context, address string, maxFetch int, filters *btcFilterParams) (BtcAnalyzeResponse, error) {
	...
	allViews, _, err := fetchBtcTxViews(ctx, address, want)
	if err != nil {
		return BtcAnalyzeResponse{}, err
	}
...
// fetchBtcTxViews получает транзакции через кэш (SQLite) + relay-цепочку.
// Порядок: forward sync → кэш (если достаточно) → backward sync через relay.
func fetchBtcTxViews(ctx context.Context, address string, want int) ([]BtcTxView, string, error) {
	views, err := fetchBtcTxViewsCached(ctx, address, want)
	return views, "cache+relay", err
}
