...
func buildBtcGraph(center string, meta BtcSummaryResponse, txViews []BtcTxView) GraphResponse {
	...
	for _, tx := range txViews {
		...
		if tx.Direction == "in" {
			// Деньги пришли → источник — первый input, который не center
			...
			// Деньги ушли → цель — первый output, который не center
	...
	// Красим узлы по риску (простая эвристика: контрагент с одной большой tx → low)
	for i := range nodes {
		if !nodes[i].IsCenter {
			// Узел с ≥3 транзакциями и высоким общим объёмом → повышаем риск
			if nodes[i].TxCount >= 5 {
				nodes[i].RiskScore = riskScore / 2
			}
		}
	}
